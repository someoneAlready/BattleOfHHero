...
public class Register {
	...
	public Register(String s) {
		...
		/*
		 * try{ socket = new Socket("127.22.22.1",12300); in = new
		 * DataInputStream(socket.getInputStream()); out = new
		 * DataOutputStream(socket.getOutputStream()); }catch(IOException el){}
		 */
		frame = new JFrame(s);
		// frame.setUndecorated(true); //将边框去除

		background = new ImageIcon(this.getClass().getClassLoader()
				.getResource("com/me/battleofhero/3.jpg"));
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (!num.equals(values)) {
				...
				password1.setText("");
					...
					&& password.getPassword().length != 0 && num.equals(values)) {
				...
				try {
					...
					// "jdbc:mysql://10.108.72.37:3306/game" , "ccl" ,
					// "zzuacmlab") ;
					conn = DriverManager.getConnection(url, user, pwd);
					PreparedStatement ps1 = conn
							.prepareStatement("select upwd from user where uname = ?");
